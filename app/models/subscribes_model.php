...
class Subscribes_model extends Model {
    ...
    public function get_deleted_subscribes_for_area($areaid) {
        ...
        $subscribes = Database::query('SELECT * FROM pretplate 
                                       WHERE id_podrucja = :areaid AND status = 0',
                                    array('areaid'=>$areaid) );
        return $subscribes;
    }
    public function get_restricted_subscribes_for_area($areaid) {
        ...
        $subscribes = Database::query('SELECT * FROM pretplate 
                                       WHERE id_podrucja = :areaid AND status = 2',
                                    array('areaid'=>$areaid) );
        return $subscribes;
    }
    ...
    public static function subscribe($userid, $areaid) {
        ...
        $ok = Database::insert('INSERT INTO pretplate(id_podrucja, id_korisnika, status) 
                                VALUES(:areaid, :userid, 1)',
    ...
    public static function restrict_access($userid, $areaid) {
        ...
        return Database::insert('UPDATE pretplate SET status = 2 
                                 WHERE id_korisnika = :userid AND 
                                       id_podrucja = :areaid',
                                array('userid'=>$userid, 
                                      'areaid'=>$areaid) );
